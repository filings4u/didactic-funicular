/* ============================================================
   SCREENINGS4U — IN-PERSON APPOINTMENT BUILDER
...
(() => {
  ...
  async function init() {
    ...
    try {
      ...
      if (id) {
        current = (data.appointments || []).find(
  ...
  function bind() {
    E.recipientType?.addEventListener(
      "change",
      () => {
        recipients();
        updateSummary();
    ...
    E.startAt?.addEventListener(
      "change",
      updateSummary
    );

    E.endAt?.addEventListener(
      "change",
      updateSummary
    );

    E.attendeeName?.addEventListener(
      "input",
      updateSummary
    );

    E.addAttendee?.addEventListener(
      "click",
      () => {
        attendees.push({
          name: "",
    ...
    E.save?.addEventListener(
      "click",
      save
    );

    E.saveBottom?.addEventListener(
      "click",
      save
    );

    E.cancelAppointment?.addEventListener(
      "click",
      cancel
    );
  }
  ...
  function defaults() {
    ...
    E.startAt.value = local(start);
    E.endAt.value = local(end);
  ...
  function recipientChanged() {
    const employer =
      E.recipientType.value === "employer";

    const item = employer
      ? employers.find(
          (value) =>
            value.id === E.recipient.value
        )
      : profiles.find(
          (value) =>
            value.id === E.recipient.value
  ...
  function fill(appointment) {
    if (E.pageTitle) {
      E.pageTitle.textContent =
        `${appointment.tracking_number || "Appointment"} — ${appointment.title || "In-Person Appointment"}`;
    }
  ...
  function payload() {
    return {
      ...
      attendee_name:
        E.attendeeName.value.trim() ||
        null,

      attendee_email:
        E.attendeeEmail.value.trim() ||
        null,

      attendee_phone:
        E.attendeePhone.value.trim() ||
        null,
      ...
      location_name:
        E.locationName?.value.trim() ||
        null,

      location_address:
        E.locationAddress?.value.trim() ||
        null,
      ...
      host_name:
        E.hostName.value.trim() ||
        null,

      host_email:
        E.hostEmail.value.trim() ||
        null,
